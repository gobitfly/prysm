...
func GetEpochData(s state.BeaconState, network string, epoch uint64, clClient *beacon.Client) (state.BeaconState, map[types.ValidatorIndex]*tracer.ValidatorEpochData, error) {
	tracer.ClearData()
	...
	for i := startSlot; i <= endSlot; i++ {
		...
		if serializedBlock != nil {
			...
			for i := uint64(0); i < sa.SyncCommitteeBits.Len(); i++ {
				...
				if !exists {
					return nil, nil, fmt.Errorf("validator public key does not exist in state")
				}
